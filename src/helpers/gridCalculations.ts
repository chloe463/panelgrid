 ...
 * and rounding up with Math.ceil. Ensures the result is at least 1 and does not exceed columnCount.
 *
 * ピクセル値をグリッド単位に変換します。セルサイズ (baseSize + gap) で割り、小数点は切り上げて整数にします。
 * 結果は最小1、最大columnCountに制限されます。
 */
export function pixelsToGridSize(pixels: number, baseSize: number, gap: number, columnCount?: number): number {
  const gridSize = Math.ceil(pixels / (baseSize + gap));
  const constrainedSize = Math.max(1, gridSize);
  return columnCount !== undefined ? Math.min(constrainedSize, columnCount) : constrainedSize;
}
